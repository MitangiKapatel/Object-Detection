...
class ImageSender():
    ...
    def __init__(self, key=12345, connect_to='tcp://127.0.0.1:5555'):
    ...
    def send_image(self, msg, image):
        ...
        if image.flags['C_CONTIGUOUS']:
            ...
            self.zmq_socket.send_array(image, self.key, msg, copy=False)
        else:
            # else make it contiguous before sending
            image = np.ascontiguousarray(image)
            self.zmq_socket.send_array(image, self.key, msg, copy=False)
        hub_reply = self.zmq_socket.recv_json()  # receive the reply message in the form of json
        return hub_reply
...
class ImageHub():
    ...
    def recv_image(self, copy=False):
        ...
        key, msg, image = self.zmq_socket.recv_array(copy=False)
        return key, msg, image
    ...
    def send_reply(self, reply_message):
        ...
        """
        self.zmq_socket.send_json(reply_message)
...
class SerializingSocket(zmq.Socket):
    ...
    def send_array(self, A, key, msg='NoName', flags=0, copy=True, track=False):
    ...
    def recv_array(self, flags=0, copy=True, track=False):
        ...
        return (md['key'], md['msg'], A.reshape(md['shape']))
